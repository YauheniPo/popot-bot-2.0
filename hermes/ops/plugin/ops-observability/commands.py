...
_USAGE = "Usage: /ops [summary|system|models|tools|costs|commands|health] [1h|24h|7d|30d]"
...
def _system_report(label: str) -> str:
...
def _summary_report(cutoff: str, label: str) -> str:
    api = _query(
...
def _models_report(cutoff: str, label: str) -> str:
    rows = _query(
...
def _tools_report(cutoff: str, label: str) -> str:
    rows = _query(
...
def _commands_report(cutoff: str, label: str) -> str:
    rows = _query(
...
def _ops_command(raw_args: str) -> str:
    ...
    if topic == "system":
        return _system_report(label)
    if topic == "summary":
        return _summary_report(cutoff, label)
    if topic in {"models", "costs"}:
        return _models_report(cutoff, label)
    if topic == "tools":
        return _tools_report(cutoff, label)
    if topic == "commands":
        return _commands_report(cutoff, label)
    return _USAGE
