...
controller (не в результат задачи) и повторяется с каждой новой попыткой,
чтобы подтвердить запрос вручную. Ошибка открытия браузера не прерывает
ожидание: авторизация могла быть начата другим локальным процессом. Успех
означает успешное SSH-подключение, а не запуск браузера или только вход в
аккаунт.
...
Pre-backup проход работает с *установленной* (старой) версией Hermes: если её
архиватор не совпадает с анкором под закреплённую версию, deploy пишет warning
и делает backup без исключения; патч применяется после установки. Отсутствие
