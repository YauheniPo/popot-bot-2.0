...
- Status: idea
- Ценность: обычные запросы остаются на выбранной бесплатной модели, а сложные
  задачи получают более сильную модель без ручного переключения.
...
- Проверка и rollback: local Docker с малым budget; удалить router config и
  восстановить исходную модельную конфигурацию.
...
- Scope: после выхода фикса в `anthropics/claude-code-action` обновить пин
  в `.github/workflows/pr-ai-review.yml` (сейчас закреплён на
